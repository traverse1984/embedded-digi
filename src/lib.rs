...
///
/// // Read one pin:
/// let val: bool = digi::read!(pin);
///
/// // Read multiple pins:
/// let (a, b): (bool, bool) = digi::read!(pin_a, pin_b);
///
/// // True if any pin matches:
/// let any_high: bool = digi::read!(any true; pin_a, pin_b);
/// let any_low: bool = digi::read!(any 0; pin_a, pin_b);
///
/// // True if all pins match:
/// let all_high: bool = digi::read!(all 1; pin_a, pin_b);
/// let all_low: bool = digi::read!(all false; pin_a, pin_b);
///
/// // Count matching pins (as u8):
/// let num_high: u8 = digi::read!(count 1; pin_a, pin_b);
/// let num_low: u8 = digi::read!(count 0; pin_a, pin_b);
///
/// // Interpret pins as an int:
/// let num: u8 = digi::read!(u8; a, b, c, d); // 0b0000abcd
/// ```
#[macro_export]
macro_rules! read {
    ($pin: expr) => {
...
///
/// // Write multiple pins:
/// digi::write!(pin_a, pin_b => true);
///
/// // Interpret pins as some bits of an int:
/// digi::write!(a, b, c => 3 bit => 0b101); // a=1, b=0, c=1
/// digi::write!(a, b, c => 5 bit => 0b11100); // a=1, b=1, c=1
///
